...
if (!$readCat) 
{	   		$readCat = read('cat', "WHERE url = 'home' AND tipo = 'menu'"); foreach ($readCat as $cat);			
...
###########################################CARROUSSEL DAS CATEGORIAS ##############################################     			
    ...
    <?php /*
         ...
         if ($cat['id_pai'] != '') 
         {
             $readArtigos = read('posts',"WHERE categoria = '$cat[id]' AND tipo = 'post' AND status = '1' ORDER BY data DESC LIMIT $inicio, $maximo");
         }else
         {
             $readArtigos = read('posts',"WHERE cat_pai = '$cat[id]' AND tipo = 'post' AND status = '1' ORDER BY data DESC LIMIT $inicio, $maximo");     
         }
        ...
        if ($cat['id_pai'] != '') 
        {
           readPaginator('posts'," WHERE categoria = '$cat[id]' AND tipo = 'post' AND status = '1' ORDER BY data DESC", $maximo, $link, $pag,'870px');
        }else
        {
